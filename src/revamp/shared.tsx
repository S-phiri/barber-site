...
export function Icon({ name, size = 16, style }: { name: IconName; size?: number; style?: React.CSSProperties }) {
  ...
  const p = paths[name];
  const isFilled = (["star", "heart", "zap", "sparkle"] as IconName[]).includes(name);
  return (
    <svg
      viewBox="0 0 24 24"
...
export function Chip({
  ...
  tone?: "default" | "amber" | "silver" | "green" | "red";
  ...
  return (
    <span className={cls} style={style}>
      {dot ? <span className={`dot ${live ? "dot-live" : ""}`} /> : null}
      {children}
    </span>
  );
}
...
type ToastItem = { id: string; title: string; body?: string; tone?: ToastTone; icon?: IconName };
type PushToast = (t: Omit<ToastItem, "id">) => void;
...
export function ToastProvider({ children }: { children: React.ReactNode }) {
  ...
  const push = useCallback<PushToast>((t) => {
    ...
    setToasts((p) => [...p, { ...t, id }]);
    window.setTimeout(() => setToasts((p) => p.filter((x) => x.id !== id)), 3600);
  }, []);
        ...
        {toasts.map((t) => (
          <div key={t.id} className={`toast ${t.tone || ""}`}>
            ...
            <div>
              ...
              {t.body ? <div style={{ fontSize: 12, color: "var(--text-2)", marginTop: 2 }}>{t.body}</div> : null}
            </div>
          </div>
        ))}
      </div>
